...
app.get('/api/admin/products', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const limit = parseInt(req.query.limit as string) || 5; // Make sure this matches your frontend
    ...
    if (req.query.subcategory) {
      query.subcategory = req.query.subcategory as string;
    }
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    if (typeof totalAmount !== 'number') {
      ...
      const productMap = products.reduce((acc: any, p) => ({ ...acc, [p._id.toString()]: p.price }), {});
      calculatedTotal = items.reduce((sum: number, item: any) => sum + (item.quantity * productMap[item.product]), 0);
    }
