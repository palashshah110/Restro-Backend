...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.mimetype.startsWith('image/')) {
      ...
      cb(new Error('Only image files are allowed!') as never, false);
    }
  }
});
...
app.get('/api/baskets', async (req, res) => {
  try {
    const { size, category }:any = req.query;
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    if (orderType === 'gift_basket' && baskets.length > 0) {
      basketData = baskets.map((basket:any) => ({
        basketNumber: basket.basketNumber,
        items: basket.items,
        total: basket.total || basket.items.reduce((sum:any, item:any) => sum + (item.quantity * item.price), 0),
      }));
